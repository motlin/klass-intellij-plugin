...
public class KlassMemberReference extends PsiPolyVariantReferenceBase<PsiElement> {
	...
	public static final ImmutableList<String> VALID_PROPERTIES = Lists.immutable.with("valid", "validFrom", "validTo");
	public static final ImmutableList<String> SYSTEM_PROPERTIES = Lists.immutable.with(
		"system",
		"systemFrom",
		"systemTo"
	);
	public static final ImmutableList<String> AUDIT_PROPERTIES = Lists.immutable.with(
		"createdById",
		"createdOn",
		"lastUpdatedById"
	);
	...
	public ResolveResult[] multiResolve(boolean incompleteCode) {
		...
		if (parent instanceof KlassExpressionThisMember) {
			...
			if (resolve instanceof KlassKlass klass) {
				return this.getKlassResolveResults(klass);
			}
			if (resolve instanceof KlassEnumeration enumeration) {
			...
			KlassAssociationEnd associationEnd = PsiTreeUtil.getParentOfType(this.myElement, KlassAssociationEnd.class);
		...
		} else if (parent instanceof KlassProjectionLeafNode projectionLeafNode) {
			...
			if (projectionNodeParent instanceof KlassProjection projection) {
				...
				if (resolved instanceof KlassClassifier classifier) {
					return this.getClassifierResolveResults(classifier);
				}
				if (resolved instanceof KlassClassModifier) {
					return new ResolveResult[] { new PsiElementResolveResult(resolved) };
				}
			} else if (projectionNodeParent instanceof KlassProjectionAssociationEndNode associationEndNode) {
				...
				projectionNodeParent instanceof KlassProjectionParameterizedPropertyNode parameterizedPropertyNode
			) {
	...
	public Object[] getVariants() {
		...
		} else if (grandparent instanceof KlassProjection projection1) {
			KlassClassifierName classifierName = projection1.getClassifierName();
			PsiReference classifierReference = classifierName.getReference();
			PsiElement resolved = classifierReference.resolve();

			if (resolved instanceof KlassKlass klass) {
				return this.getKlassMemberLookups(klass);
			}
			if (resolved instanceof KlassInterface interface1) {
				return this.getInterfaceMemberLookups(interface1);
			}
			return new Object[] {};
	...
	private ResolveResult[] getClassifierResolveResults(KlassClassifier classifier) {
		if (classifier instanceof KlassKlass klass) {
			return getKlassResolveResults(klass);
		}
		if (classifier instanceof KlassInterface interface1) {
			return getInterfaceResolveResults(interface1);
		}
		throw new AssertionError();
	}
	...
	private ResolveResult[] getKlassResolveResults(KlassKlass klass) {
		...
		if (extendsClause != null) {
			...
			if (resolvedSuperClass != null) {
				ResolveResult[] superClassResolvedResults = this.getKlassResolveResults(
					(KlassKlass) resolvedSuperClass
				);
	...
	private void addMacroPropertyLookups(List<KlassClassModifier> classModifierList, List<Object> lookups) {
		for (KlassClassModifier modifier : classModifierList) {
			String text = modifier.getText();
			ImmutableList<String> macroProperties = switch (text) {
	...
	@NotNull private ResolveResult[] getTemporalReference(String temporalKeyword, List<KlassClassModifier> classModifierList) {
		return classModifierList
			.stream()
			.filter(
				(classModifier) ->
					classModifier.getText().equals(temporalKeyword) || classModifier.getText().equals("bitemporal")
			)
			.map(PsiElementResolveResult::new)
			.toArray(ResolveResult[]::new);
	}

	@NotNull private ResolveResult[] getModifierReference(String modifierKeyword, List<KlassClassModifier> classModifierList) {
		return classModifierList
			.stream()
