...
public class AnnotatorKlassVisitor extends KlassVisitor {
  ...
  public void visitAssociation(@NotNull KlassAssociation klassAssociation) {
    List<KlassAssociationEnd> associationEndList =
        klassAssociation.getAssociationBlock().getAssociationBody().getAssociationEndList();
  ...
  public void visitCriteriaOperator(@NotNull KlassCriteriaOperator criteriaOperator) {
    ...
    boolean actualMany =
        ListAdapter.adapt(possibleTargetTypes)
            .allSatisfy(type -> type.getMultiplicity().isToMany());
  ...
  @Override
  public void visitExpressionVariableName(
  ...
  public void visitProjectionParameterizedPropertyNode(
    ...
    if (parameterNameList.size() == propertyParameterDeclarations.size()) {
      this.typeCheckPassedParameters(
          parameterNameList, propertyParameterDeclarations, projectionParameterDeclarations);
    } else {
      String message =
          String.format(
  ...
  public void visitServiceProjectionClause(@NotNull KlassServiceProjectionClause projectionClause) {
    ...
    List<KlassParameterName> parameterNameList =
        parameterNames == null ? Arrays.asList() : parameterNames.getParameterNameList();
    ...
    if (projection != null) {
      ...
      if (parameterNameList.size() == projectionParameterDeclarations.size()) {
        this.typeCheckPassedParameters(
            parameterNameList, projectionParameterDeclarations, serviceParameterDeclarations);
      } else {
        String message =
            String.format(
                "Projection '%s' cannot be applied to given types;%nrequired: '%s'%nfound:'%s'%nreason: actual and formal argument lists differ in length.",
  ...
  public void typeCheckPassedParameters(
    ...
    for (int i = 0; i < parameterNameList.size(); i++) {
      ...
      KlassParameterDeclaration serviceParameterDeclaration =
          projectionParameterDeclarations.get(i);
  ...
  private List<Type> getPossibleTypes(KlassExpressionValue expressionValue) {
    if (expressionValue instanceof KlassExpressionLiteral) {
      ...
      return AnnotatorKlassVisitor.getExpressionLiteralTypes(
          expressionLiteral, Multiplicity.ONE_TO_ONE);
    }
    ...
    if (expressionValue instanceof KlassExpressionLiteralsParens
      ...
      return AnnotatorKlassVisitor.getExpressionLiteralTypes(
          expressionLiteralList.get(0), Multiplicity.ZERO_TO_MANY);
    }

    if (expressionValue instanceof KlassExpressionNativeValue) {
      KlassExpressionNativeValue expressionNativeValue =
          (KlassExpressionNativeValue) expressionValue;
    ...
    if (expressionValue instanceof KlassExpressionMemberName) {
      ...
      if (resolve instanceof KlassPrimitiveTypeProperty) {
        ...
        MutableList<Type> result =
            Lists.mutable.with(new Type(DataTypeType.PRIMITIVE_TYPE, dataTypeText, multiplicity));
      ...
      if (resolve instanceof KlassEnumerationLiteral) {
        ...
        KlassEnumeration enumeration =
            PsiTreeUtil.getParentOfType(enumerationLiteral, KlassEnumeration.class);
  ...
  public static List<Type> getExpressionLiteralTypes(
      KlassExpressionLiteral expressionLiteral, Multiplicity multiplicity) {
    KlassBooleanLiteral booleanLiteral = expressionLiteral.getBooleanLiteral();
    if (booleanLiteral != null) {
      return Collections.singletonList(
          new Type(DataTypeType.PRIMITIVE_TYPE, "Boolean", multiplicity));
    }
    ...
    if (stringLiteralNode != null) {
      return Collections.singletonList(
          new Type(DataTypeType.PRIMITIVE_TYPE, "String", multiplicity));
    }
