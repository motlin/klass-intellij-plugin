...
public class AnnotatorKlassVisitor extends KlassVisitor {
	...
	public static final ImmutableList<String> TEMPORAL_PROPERTY_MODIFIERS = Lists.immutable.with(
		"validTemporal",
		"systemTemporal",
		"bitemporal"
	);
	...
	public void visitAssociation(@NotNull KlassAssociation klassAssociation) {
		List<KlassAssociationEnd> associationEndList = klassAssociation
			.getAssociationBlock()
			.getAssociationBody()
	...
	public void visitAssociationEnd(@NotNull KlassAssociationEnd klassAssociationEnd) {
		if (klassAssociationEnd.getMultiplicity() == null) {
			this.annotationHolder.newAnnotation(HighlightSeverity.ERROR, "Expected multiplicity")
	...
	public void visitCriteriaOperator(@NotNull KlassCriteriaOperator criteriaOperator) {
		KlassExpressionValue sourceExpressionValue = criteriaOperator.getSourceExpressionValue().getExpressionValue();
		KlassExpressionValue targetExpressionValue = criteriaOperator.getTargetExpressionValue().getExpressionValue();
		...
		boolean actualMany = ListAdapter.adapt(possibleTargetTypes).allSatisfy((type) ->
	...
	public void visitInterface(@NotNull KlassInterface klassInterface) {
		this.annotateDuplicateProperties(klassInterface.getInterfaceBlock().getInterfaceBody().getMemberList());
	}
	...
	private void annotateDuplicateProperties(List<KlassMember> propertyList) {
		Map<String, Long> propertyCountByName = propertyList
			.stream()
	...
	private void annotateUnresolvableReference(PsiElement elementWithReference) {
		...
		if (reference != null && isUnresolved(reference)) {
			...
			this.annotationHolder.newAnnotation(HighlightSeverity.ERROR, message).range(elementWithReference).create();
		}
	...
	public void visitMemberName(@NotNull KlassMemberName propertyName) {
		...
		if (resolveResults.length == 0) {
			...
			this.annotationHolder.newAnnotation(HighlightSeverity.ERROR, message).range(propertyName).create();
			return;
		}
		PsiElement resolved = resolveResults[0].getElement();
		if (
			resolved instanceof KlassPrimitiveTypeProperty
	...
	public void visitParameterDeclaration(@NotNull KlassParameterDeclaration parameterDeclaration) {
		...
		if (dataTypeDeclaration == null || dataTypeDeclaration.getMultiplicity() == null) {
			...
			this.annotationHolder.newAnnotation(HighlightSeverity.ERROR, message).range(parameterDeclaration).create();
		}
	}
	...
	@Override
	public void visitProjectionParameterizedPropertyNode(
		@NotNull KlassProjectionParameterizedPropertyNode projectionParameterizedPropertyNode
	) {
		...
		List<KlassParameterName> parameterNameList = projectionParameterizedPropertyNode
		...
		MutableList<KlassParameterDeclaration> projectionParameterDeclarations = ListAdapter.adapt(parameterNameList)
	...
	public void visitServiceProjectionClause(@NotNull KlassServiceProjectionClause projectionClause) {
		...
		if (projection != null) {
			...
			MutableList<KlassParameterDeclaration> projectionParameterDeclarations = Optional.ofNullable(
				projection.getParameterDeclarationsParens()
			)
	...
	public void typeCheckPassedParameters(
		...
		for (var i = 0; i < parameterNameList.size(); i++) {
			...
			List<Type> serviceParameterTypes = this.getParameterDeclarationType(serviceParameterDeclaration);
			List<Type> projectionParameterTypes = this.getParameterDeclarationType(projectionParameterDeclaration);
	...
	public void annotateCannotResolve(@NotNull PsiElement psiElement) {
		...
		if (isUnresolved(reference)) {
			...
			this.annotationHolder.newAnnotation(HighlightSeverity.ERROR, message).range(psiElement).create();
		}
	}
	...
	private List<Type> getPossibleTypes(KlassExpressionValue expressionValue) {
		...
		if (
			expressionValue instanceof KlassExpressionLiteralsParens
		...
		if (expressionValue instanceof KlassExpressionMemberName expressionMemberName) {
			...
			if (resolve instanceof KlassPrimitiveTypeProperty primitiveTypeProperty) {
				...
				MutableList<Type> result = Lists.mutable.with(
					new Type(DataTypeType.PRIMITIVE_TYPE, dataTypeText, multiplicity)
				);
	...
	@Nullable private List<Type> getParameterDeclarationType(KlassParameterDeclaration parameterDeclaration) {
		KlassDataTypeDeclaration dataTypeDeclaration = parameterDeclaration.getDataTypeDeclaration();
		...
		return Collections.singletonList(
			new Type(dataTypeType, dataType.getText(), this.getMultiplicity(multiplicity))
		);
	}
